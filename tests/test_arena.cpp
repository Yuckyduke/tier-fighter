...
void testStreakBonus() {
    ...
    if (r) {
        ...
        checkEq(r->score, (S.streakThreshold - 1) * S.pointsPerKO, "score is KOs only, no bonus");
    }
...
void testKnockoffResetsStreak() {
    ...
    if (r) {
        ...
        checkEq(r->score, scoreBeforeKnockoff + S.pointsPerKnockoff, "knock-off penalty applied");
...
void testKnockedOffMovesToDifferentStage() {
    ...
    if (after) {
        ...
        check(after->stageId != leftStage, "routed to a DIFFERENT stage than the one just left");
    }
}
...
void testLonelyStageConsolidates() {
    ...
    idleStep(arena, A.lonelyGraceFrames + 2);
    const PlayerRecord *lonely = arena.player(static_cast<uint32_t>(A.playersPerStage));
...
void testLeaderboard() {
    ...
    bool idOrdered = true;
    for (int i = 1; i < tn; ++i) {
        if (tiedBoard[i - 1].score == tiedBoard[i].score && tiedBoard[i - 1].id > tiedBoard[i].id) {
            idOrdered = false;
        }
    }
