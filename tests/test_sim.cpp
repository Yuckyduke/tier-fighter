...
void testShieldBreakAndDizzy() {
    ...
    for (int f = 0; f < 1200; ++f) {
        ...
        if (gs.players[0].state == ActionState::Dizzy) { sawDizzy = true; break; }
    }
...
void testShieldMinimumHold() {
    ...
    for (int f = 0; f < S.minHoldFrames + S.releaseFrames + 4; ++f) tick(0);
    check(gs.players[0].state == ActionState::Idle,
...
void testRollAndSpotDodge() {
    ...
    auto escapeFrom = [](int8_t sx, int8_t sy) {
        ...
        for (int f = 0; f < 3; ++f) tick(0, 0);   // settle into shield
        const fx startX = gs.players[0].x;
        tick(sx, sy);

        struct R { ActionState st; fx moved; int invulnFrames; };
...
void testEscapeInvulnerabilityIsWindowed() {
    ...
    check(E.rollInvulnFrames < E.rollFrames,
          "roll is not invulnerable for its whole duration");
    check(E.dodgeInvulnStart > 0, "spotdodge startup is vulnerable");
    check(E.dodgeInvulnFrames < E.dodgeFrames,
          "spotdodge is not invulnerable throughout");
}
...
void testShieldIsPerCharacter() {
    ...
    check(bShield.drainPerFrame < sShield.drainPerFrame,
          "bruiser's shield drains slower");
    check(bShield.releaseFrames > sShield.releaseFrames,
          "bruiser pays more release lag");
    ...
    check(sg.players[0].state == ActionState::Shield ||
          sg.players[0].state == ActionState::ShieldOn, "scout can shield");
    check(bg.players[0].state == ActionState::Shield ||
          bg.players[0].state == ActionState::ShieldOn, "bruiser can shield");
}
