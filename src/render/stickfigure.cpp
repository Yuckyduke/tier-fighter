...
void drawPose(const pose::Pose& p, const Frame& f, Color body, float thickness) {
    ...
    const Vector2 shoulder{
        hip.x + std::sin(leanR) * shoulderLen * static_cast<float>(dir),
    ...
    const Vector2 handB  = offsetFrom(elbowB, p.shoulderBackDeg + p.elbowBackDeg,
                                      fore, dir);
    ...
    const Vector2 handF  = offsetFrom(elbowF, p.shoulderFrontDeg + p.elbowFrontDeg,
                                      fore, dir);
    ...
    const Color backColor{
        static_cast<unsigned char>(body.r / 2 + 20),
        static_cast<unsigned char>(body.g / 2 + 20),
        static_cast<unsigned char>(body.b / 2 + 20),
        body.a};
    ...
    // symmetrical pose.
    DrawCircleV(Vector2{head.x + headR * 0.42f * static_cast<float>(dir),
                        head.y - headR * 0.12f},
                headR * 0.17f,
                Color{20, 22, 28, 255});
}
