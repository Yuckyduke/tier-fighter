...
void printPose(const char* label, const pose::Pose& p) {
    std::printf("    // %s\n    P(%g, %g, %g, %g, %g, %g, %g, %g, %g, %g, %g)\n",
                label, p.leanDeg, p.headOffsetY,
                p.shoulderFrontDeg, p.elbowFrontDeg,
                p.shoulderBackDeg, p.elbowBackDeg,
                p.hipFrontDeg, p.kneeFrontDeg,
                p.hipBackDeg, p.kneeBackDeg, p.crouch);
}
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_LEFT_ALT)) {
            ...
            blender.defaultBlendFrames =
                (blender.defaultBlendFrames >= 12) ? 2
                                                   : blender.defaultBlendFrames + 2;
        }
        ...
        if (IsKeyPressed(KEY_COMMA)) { frame = frame > 0 ? frame - 1 : 0; playing = false; }
        ...
        if (IsKeyPressed(KEY_D)) {
            std::printf("\n// --- %s (%s keyframe) ---\n",
                        pose::stateLabel(st), editingB ? "b" : "a");
            printPose("a", an.a);
        ...
        if (showGrid) {
            ...
            for (int i = 1; i <= 4; ++i) {
                ...
                DrawLineEx(Vector2{0, y}, Vector2{kScreenW, y}, 1.0f,
                           Color{38, 42, 50, 255});
            }
        }
        ...
        const pose::Pose shown = blendOn
            ? pose::poseBlended(blender, st, animFrame, 0)
        ...
        std::snprintf(line, sizeof(line), "blend %s  (%d fr)%s",
                      blendOn ? "ON" : "off", blender.defaultBlendFrames,
                      blender.blendLen > 0 ? "   BLENDING" : "");
        DrawText(line, kScreenW - 260, 22, 18,
                 blender.blendLen > 0 ? Color{255, 220, 120, 255}
                                      : Color{130, 145, 165, 255});
        ...
        for (int i = 0; i < kFieldCount; ++i) {
            ...
            DrawText(line, 24, y, 18,
                     sel ? Color{255, 220, 120, 255} : Color{130, 140, 155, 255});
            y += 22;
        }

        const char* help =
            "M  single <-> SEQUENCE mode   L  loop   B  blending on/off\n"
