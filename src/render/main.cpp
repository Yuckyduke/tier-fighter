...
constexpr KeyBinding kP1{
    ...
    KEY_SPACE, KEY_F, KEY_G,
    0,
};

constexpr KeyBinding kP2{
    KEY_LEFT, KEY_RIGHT, KEY_UP, KEY_DOWN,
    KEY_RIGHT_SHIFT, KEY_PERIOD, KEY_SLASH,
    1,
};
...
void drawStage(const Stage& s, const ViewTransform& v) {
    ...
    DrawRectangleLines(
        static_cast<int>(v.sx(s.blastLeft)),
        static_cast<int>(v.sy(s.blastTop)),
        static_cast<int>(v.len(s.blastRight - s.blastLeft)),
        static_cast<int>(v.len(s.blastBottom - s.blastTop)),
        Color{60, 20, 20, 255});
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        DrawRectangleLines(
            static_cast<int>(v.sx(boxLeft)),
            static_cast<int>(v.sy(s.groundY - L.grabReachUp)),
            static_cast<int>(v.len(L.grabReachX)),
            static_cast<int>(v.len(L.grabReachUp + L.grabReachDown)),
            Color{90, 140, 200, 110});
    }
}
...
void drawPlayer(const Player& p, int index, const ViewTransform& v, bool showBoxes) {
    ...
    if (p.state == ActionState::Hitstun) {
        c = Color{250, 240, 120, 255};
    }
    ...
    DrawRectangleLines(static_cast<int>(left), static_cast<int>(top),
                       static_cast<int>(w), static_cast<int>(h),
                       Color{120, 255, 140, 160});
    ...
    const bool attacking = (p.state == ActionState::AttackGround ||
    ...
    if (p.charging) {
        const float t = static_cast<float>(p.chargeFrames) /
                        static_cast<float>(config::kSmash.maxChargeFrames);
    ...
    const bool live = resolvedFrame >= atk.startup &&
                      resolvedFrame < atk.startup + atk.active;
    if (!live) return;
...
void drawPlayerHud(const Player& p, int index, int x, int y) {
    const Color labelColor = (index == 0) ? Color{120, 190, 250, 255}
                                          : Color{250, 150, 120, 255};

    char line[256];
    std::snprintf(line, sizeof(line), "P%d  %3d%%", index + 1,
                  fx_to_int(p.damage));
    ...
    // actually works.
    std::snprintf(line, sizeof(line), "self %+.2f %+.2f",
                  fx_to_float(p.selfVelX), fx_to_float(p.selfVelY));
    DrawText(line, x, y + 50, 16, Color{150, 200, 150, 255});

    std::snprintf(line, sizeof(line), "kb   %+.2f %+.2f",
                  fx_to_float(p.kbVelX), fx_to_float(p.kbVelY));
    ...
    if (p.attackId != config::ATK_NONE) {
        ...
        row += 20;
        if (p.charging) {
            std::snprintf(line, sizeof(line), "  CHARGE %d/%d",
                          p.chargeFrames, config::kSmash.maxChargeFrames);
    ...
    if (p.ledgeSide >= 0) {
        std::snprintf(line, sizeof(line), "on ledge (%s)  hang %u",
                      p.ledgeSide == 0 ? "L" : "R", p.ledgeHangFrames);
    ...
    if (p.pendingKOs > 0) {
        DrawText("KO! (full heal)", x, row, 16, Color{120, 255, 160, 255});
    }
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (paused) {
            DrawText("PAUSED  (N = step one frame)", kScreenW / 2 - 170,
                     24, 22, Color{255, 220, 120, 255});
        } else if (slowMo) {
        ...
        char footer[128];
        std::snprintf(footer, sizeof(footer), "tick %u   checksum %08x   %d fps",
                      gs.tick, checksum(gs), GetFPS());
        DrawText(footer, kScreenW - 330, kScreenH - 28, 16,
