...
constexpr Pose P(float lean, float headY,
                 float shF, float elF, float shB, float elB,
                 float hipF, float knF, float hipB, float knB,
                 float crouch) {
...
constexpr Anim Cycle(Pose a, Pose b, int frames) {
    return Anim{a, b, frames, true, true};
}
...
float poseDistance(const Pose& a, const Pose& b) {
    ...
    return d(a.leanDeg, b.leanDeg) +
           d(a.shoulderFrontDeg, b.shoulderFrontDeg) +
           d(a.elbowFrontDeg, b.elbowFrontDeg) +
           d(a.shoulderBackDeg, b.shoulderBackDeg) +
           d(a.elbowBackDeg, b.elbowBackDeg) +
           d(a.hipFrontDeg, b.hipFrontDeg) +
           d(a.kneeFrontDeg, b.kneeFrontDeg) +
           d(a.hipBackDeg, b.hipBackDeg) +
           d(a.kneeBackDeg, b.kneeBackDeg) +
           d(a.crouch, b.crouch) * 100.0f +
...
Pose poseBlended(Blender& b, ActionState st, int stateFrame, uint8_t attackId) {
    ...
    if (st != b.lastState) {
        const Pose was = b.blendLen > 0
            // Already mid-blend: capture the blended result so a rapid A->B->C
            // chain does not snap back to A's pose.
            ? [&] {
                const float t = ease(static_cast<float>(b.blendFrame) /
                                     static_cast<float>(b.blendLen));
