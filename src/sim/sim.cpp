...
uint16_t computeGrabHold(const Player& victim, const config::Grab& G) {
    int32_t frames = G.holdBaseFrames +
                     fx_to_int(fx_mul(victim.damage, G.holdPerDamage));
...
void startThrow(GameState& gs, int grabberIdx, uint8_t dir) {
    ...
    const fx weightFactor = kFxOne +
        fx_mul(fighterOf(v).body.weight, kThrow.weightDurationScale);
...
void releaseThrow(GameState& gs, int grabberIdx) {
    ...
    switch (g.throwDir) {
        case 2: damage = kThrow.damageB; growth = kThrow.growthB;
                angle = kThrow.angleB; break;
        case 3: damage = kThrow.damageU; growth = kThrow.growthU;
                angle = kThrow.angleU; break;
        case 4: damage = kThrow.damageD; growth = kThrow.growthD;
                angle = kThrow.angleD; break;
        default: break;
    }
...
void resolveGrab(GameState& gs, int grabberIdx) {
    ...
    for (int t = 0; t < kMaxPlayers; ++t) {
        ...
        if (best < 0 || distSq < bestDist) { best = t; bestDist = distSq; }
    }
...
bool isShieldState(const Player& p) {
    return p.state == ActionState::ShieldOn ||
           p.state == ActionState::Shield ||
...
bool blocksShieldRegen(const Player& p) {
    return isShieldState(p) ||
           p.state == ActionState::ShieldBroken ||
...
void breakShield(Player& p) {
    ...
    int32_t dizzy = kShieldBreak.dizzyBase +
                    fx_to_int(fx_mul(p.damage, kShieldBreak.dizzyPerDamage));
...
void startRoll(Player& p, int8_t dir) {
    ...
    enterState(p, dir == p.facing ? ActionState::RollForward
...
bool checkGroundActions(Player& p, const Input& in, const Input& prev) {
    ...
    if (in.held(BtnShield) && in.pressed(BtnAttack, prev) &&
...
void checkBlastZones(GameState& gs, int idx) {
    ...
    if (p.grabPartner < kMaxPlayers) {
        ...
        if (other.grabPartner == static_cast<uint8_t>(idx)) {
            ...
            other.throwDir = 0;
            other.throwFrames = 0;
            if (other.state == ActionState::GrabHold ||
                other.state == ActionState::Pummel ||
                other.state == ActionState::Throwing ||
                other.state == ActionState::Grabbed ||
...
void resolveGrabPairs(GameState& gs) {
    for (int i = 0; i < kMaxPlayers; ++i) {
        ...
        if (v.state != ActionState::Grabbed && v.state != ActionState::Thrown) {
            continue;
        }
    ...
    for (int i = 0; i < kMaxPlayers; ++i) {
        ...
        switch (p.state) {
            ...
            case ActionState::GrabHold: {
                ...
                if (dir != 0) { startThrow(gs, i, dir); break; }
                ...
                if (!in.held(BtnShield)) { breakGrab(gs, i); break; }
                ...
                if (fx_abs(v.x - p.x) > G.maxHoldDistX ||
                    fx_abs(v.y - p.y) > G.maxHoldDistY) {
                    breakGrab(gs, i);
                }
                break;
            }
            ...
            case ActionState::Shield: {
                ...
                if (p.shieldHealth <= 0) { breakShield(p); break; }
            ...
            case ActionState::ShieldStun:
                ...
                if (p.shieldStunFrames == 0) {
                    ...
                    if (in.held(BtnShield) && p.shieldHealth > 0) {
                        ...
                        p.groundActionFrames =
                            static_cast<uint8_t>(F.shield.releaseFrames);
                        enterState(p, ActionState::ShieldOff);
                    }
                }
                break;
        ...
        const bool wasGrounded = (p.state == ActionState::Idle ||
                                  ...
                                  p.state == ActionState::ShieldOn ||
                                  p.state == ActionState::Shield ||
                                  p.state == ActionState::ShieldOff ||
                                  p.state == ActionState::ShieldStun ||
                                  p.state == ActionState::Dizzy ||
                                  p.state == ActionState::RollForward ||
                                  p.state == ActionState::RollBack ||
                                  p.state == ActionState::SpotDodge ||
                                  p.state == ActionState::Grabbing ||
                                  p.state == ActionState::GrabHold ||
                                  p.state == ActionState::Pummel ||
                                  p.state == ActionState::Throwing ||
                                  p.state == ActionState::GrabRelease ||
