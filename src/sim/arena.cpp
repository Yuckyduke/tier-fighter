...
const PlayerRecord *Arena::player(uint32_t playerId) const {
    for (uint32_t i = 0; i < playerCount_; ++i) {
        if (players_[i].id == playerId && players_[i].status != PlayerStatus::Absent) {
            return &players_[i];
        }
    }
    return nullptr;
...
int Arena::liveStageCount() const {
    int n = 0;
    for (const StageInstance &s : stages_)
        if (s.live) ++n;
    return n;
}
    ...
    for (int i = 0; i < kMaxStages; ++i) {
        ...
        for (int slot = 0; slot < kPlayersPerStage; ++slot) {
            ...
            for (int k = 0; k < kos; ++k) {
                rec->kos++;
                rec->currentStreak++;
                if (rec->currentStreak > rec->bestStreak) { rec->bestStreak = rec->currentStreak; }
                ...
                if (S.streakThreshold > 0 && rec->currentStreak % S.streakThreshold == 0) {
...
int Arena::leaderboard(PlayerRecord *out, int maxEntries) const {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        while (j >= 0) {
            ...
            if (key.score != c.score)
                keyIsBetter = key.score > c.score;
            else if (key.netDelta() != c.netDelta())
                keyIsBetter = key.netDelta() > c.netDelta();
            else if (key.bestStreak != c.bestStreak)
                keyIsBetter = key.bestStreak > c.bestStreak;
            else if (key.knockoffs != c.knockoffs)
                keyIsBetter = key.knockoffs < c.knockoffs;
            else
                keyIsBetter = key.id < c.id;
